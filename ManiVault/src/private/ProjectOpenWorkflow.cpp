...
Group ProjectOpenWorkflow::makeRecipe()
{
	...
	return Group{
        ...
        QSyncTask([&] {
            ...
            try {
                openWorkspaceJson(*projectOpenContext);
            }
            catch (const std::exception& e) {
                projectOpenContext->_errorMessage = QString::fromUtf8(e.what());
            }
...
void ProjectOpenWorkflow::handleDone(QtTaskTree::DoneWith status)
{
    ...
    if (auto currentProject = mv::projects().getCurrentProject()) {
	    if (const auto result = resultAs<ProjectOpenResult>()) {
	        ...
	        const auto successText  = (duration < 1000) ? QString("%1 opened successfully in %2 ms").arg(currentProject->getFilePath()).arg(duration) : QString("%1 opened successfully in %2 s").arg(currentProject->getFilePath()).arg(duration / 1000.0, 0, 'f', 1);
        ...
        if (getOperationContext()->hasErrors()) {
            const auto errorText = "Project opened with errors: " + getOperationContext()->combinedErrorMessage();
            help().addNotification("Project opened with errors", errorText, StyledIcon("exclamation-triangle"));
            qDebug() << errorText;
        }
    }
