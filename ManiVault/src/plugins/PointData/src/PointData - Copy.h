...
public:
    ...
    public:
        ...
        StorageConfiguration(const std::string storageTypeName, T)
            : Configuration::Configuration(storageTypeName, T())
        {
        ...
        std::string storageTypeName() const
        {
            return storageType;
        }
        ...
        {
            dataType = SpecialDataTypeNames[SET_lossless];
        }
...
public:
   ...
   */
    ...
    */
    ...
    void setStorageConfiguration(const StorageConfiguration& config)
    {
        _variantOfMatrices.setConfiguration(config);
    }
...
private:
    ...
    bool tryToSetStorageType(const std::string& matrixType, const T minVal, const T maxVal)
    {
        if (tryToSetStorageType<T, Head>(minVal, maxVal))
            return true;
        else
            return tryToSetStorageType<T, Tail...>(minVal, maxVal);
    }


    template <typename T>
    bool setDataStorageType(StorageConfiguration storageConfiguration, std::vector<T>& values)
    {
        ...
        if (!storageConfiguration.isSpecialDataType())
        {
            ...
            if (!storageTypeSet)
            {
                ...
                return setDataStoarageType(storageConfiguration, data);
            }
        }
        else
        {
            ...
            else // Find optimal elementType to store the data 
            {
                if (data.empty())
                {
                    storageConfiguration.setDataType<float>();
                    return setDataStoarageType(storageConfiguration, data);
                }
                else
                {
                    ...
                    const std::string storageType = storageConfiguration.storageTypeName();
                    if (mv::pointutils::is_integer(minVal) && mv::pointutils::is_integer(maxVal) && mv::pointutils::contains_only_integers(data))
                    {
    ...
    public:
        ...
        template <typename T>
        bool setDataAs(StorageConfiguration storageConfiguration, std::vector<T> && data, const std::size_t numDimensions)
        {
            bool storageTypeSet = setDataStorageType(storageConfiguration, data);

            if (storageTypeSet)
            {
                std::visit([&data, numDimensions](auto& matrix) {

                    matrix.moveData(std::move(data), numDimensions);
                    }, _variantOfMatrices);
            }
        ...
        {
            StorageConfiguration config(T());
            return setDataAs(config, data, numDimensions);
        }

        template <typename T>
        void setData(const std::vector<T>&data, const mv::dimension_size_t numDimensions)
        {
        ...
        bool setDataAs(StorageConfiguration storageConfiguration, RowSizeType numRows, DimensionSizeType numDimensions, std::vector<RowType> && rowPointers, std::vector<ColumnType> && colIndices, std::vector<ValueType> && values)
        {
            ...
            if (storageTypeSet)
            {
                std::visit([&rowPointers, &colIndices, &values, numRows, numDimensions](auto& matrix) {

                    matrix.moveData(numRows, numDimensions, std::move(rowPointers), std::move(colIndices), std::move(values));
                    }, _variantOfMatrices);
            }
    ...
    class POINTDATA_EXPORT Points : public mv::DatasetImpl
    {
        ...
        */
        ...
        {
            assert(points.getRawData<PointData>()->isDense()); // still need to check this for sparse matrices
    ...
    public:
        ...
        */
        ...
        bool setDataAs(const StorageConfiguration& config, std::vector<T>&& data, const std::size_t numDimensions)
        {
            ...
            getRawData<PointData>()->setDataAs(config, std::move(data), numDimensions);
        ...
        bool setDataAs(StorageConfiguration storageConfiguration, RowSizeType numRows, DimensionSizeType numDimensions, std::vector<RowType>&& rowPointers, std::vector<ColumnType>&& colIndices, std::vector<ValueType>&& values)
        {
            ...
            bool result = getRawData<PointData>()->setDataAs(storageConfiguration, numRows, numDimensions, std::move(rowPointers), std::move(colIndices), std::move(values));
        ...
        bool setDataAs(const QString& matrixType, RowSizeType numRows, DimensionSizeType numDimensions, std::vector<RowType>&& rowPointers, std::vector<ColumnType>&& colIndices, std::vector<ValueType>&& values)
        {
            ...
            bool result = getRawData<PointData>()->setDataAs(matrixType, numRows, numDimensions, std::move(rowPointers), std::move(colIndices), std::move(values));
        ...
        bool isDense() const
        {
            return getRawData<PointData>()->isDense();
        }
