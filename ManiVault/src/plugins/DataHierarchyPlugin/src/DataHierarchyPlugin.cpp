...
ScriptTriggerActions DataHierarchyPluginFactory::getScriptTriggerActions(const mv::Datasets& datasets) const
{
    ...
    if (areAllDatasetsOfTheSameType(datasets, PointType) && datasets.count() >= 2) {
        auto scriptTriggerAction = new ScriptTriggerAction(nullptr, std::make_shared<PythonScript>("Stitch", Script::Type::Transformation, QUrl(), datasets), "Transformation/Stitch");

        scriptTriggerActions << scriptTriggerAction;
    }

    if (datasets.count() == 1 && datasets.first()->getDataType() == PointType) {
        scriptTriggerActions << new ScriptTriggerAction(nullptr, std::make_shared<PythonScript>("Export BIN", Script::Type::Writer, QUrl(), datasets), "Export/BIN");
        scriptTriggerActions << new ScriptTriggerAction(nullptr, std::make_shared<PythonScript>("Export PNG", Script::Type::Writer, QUrl(), datasets), "Export/JSON");
        scriptTriggerActions << new ScriptTriggerAction(nullptr, std::make_shared<PythonScript>("Average", Script::Type::Analysis, QUrl(), datasets), "Analysis/Average");
        scriptTriggerActions << new ScriptTriggerAction(nullptr, std::make_shared<PythonScript>("Min", Script::Type::Analysis, QUrl(), datasets), "Analysis/Min");
        scriptTriggerActions << new ScriptTriggerAction(nullptr, std::make_shared<PythonScript>("Max", Script::Type::Analysis, QUrl(), datasets), "Analysis/Max");
    }
